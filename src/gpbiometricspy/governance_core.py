...
def create_gazepoint_analysis_decision_log(study_id=None,analyst=None,description=None):
 d=pd.DataFrame({c:pd.Series(dtype='object') for c in _LOG_COLS});d.attrs.update(study_id=study_id,analyst=analyst,description=description,created_at=datetime.now(timezone.utc).isoformat(),package_version='0.1.4',gazepoint_analysis_decision_log=True);return d
