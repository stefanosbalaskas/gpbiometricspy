...
!!! tip "Run the complete workflow"
    The repository contains an executable Python-native companion at `examples/hands-on/end-to-end-eda-research.py`. It is executed by documentation CI, so the documented path is checked against the package rather than maintained as untested pseudocode. The separate `examples/tutorials/` directory remains the frozen 26 R-companion tutorial surface.
