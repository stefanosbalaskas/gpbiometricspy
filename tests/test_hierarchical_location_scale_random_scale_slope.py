...
import gpbiometricspy.hierarchical_location_scale_random_scale_slope as rs


@pytest.fixture(scope="module")
def fitted_case():
    data = rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
        n_groups=10,
        observations_per_group=7,
        tau_log_scale_slope=0.28,
        seed=42,
    )
    result = rs.fit_gazepoint_hierarchical_location_scale_random_scale_slope(
        data,
        "outcome",
        "participant",
        mean_cols=["x"],
...
def test_fit_prediction_summary_certificate_and_immutability(fitted_case):
    ...
    assert result.scale_random_slope_col == "x"
    ...
    assert result.tau_log_scale_intercept > 0
    assert result.tau_log_scale_slope > 0
    assert len(result.random_effects) == 10

    seen = data.iloc[[0]].copy()
    unseen = seen.copy()
    unseen["participant"] = "NEW"
    pop = rs.predict_gazepoint_hierarchical_location_scale_random_scale_slope(
        result, seen, include_random_effects=False
    )
    p_seen = rs.predict_gazepoint_hierarchical_location_scale_random_scale_slope(result, seen)
    p_unseen = rs.predict_gazepoint_hierarchical_location_scale_random_scale_slope(result, unseen)
    custom = seen.rename(columns={"participant": "subject"})
    p_custom = rs.predict_gazepoint_hierarchical_location_scale_random_scale_slope(
    ...
    assert np.isfinite(p_seen.random_log_scale_effect.iloc[0])

    summary = rs.summarize_gazepoint_hierarchical_location_scale_random_scale_slope(result)
    assert summary["class"] == "gazepoint_hierarchical_location_scale_random_scale_slope_summary"
    assert summary["random_effect_covariance"].shape == (3, 3)
    assert set(summary["fixed_effects"]["equation"]) == {"location", "log_scale"}

    cert = rs.create_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(result)
    ...
    assert rs.validate_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(
        result, reordered
    )
    shuffled = replace(
        result,
    ...
    assert (
        rs.create_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(shuffled)[
            "payload"
        ]["random_effects_sha256"]
        == cert["payload"]["random_effects_sha256"]
    )
    mutated_re = result.random_effects.copy()
    mutated_re.loc[0, "log_scale_slope_re"] += 0.2
    mutated = replace(result, random_effects=mutated_re)
    assert not rs.validate_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(
        mutated, cert
    )
...
def test_input_guardrails_and_scale_random_slope_identifiability():
    data = rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
    ...
    for bad in (2, 12, 3.5):
        with pytest.raises(ValueError, match="quadrature_points"):
            rs.fit_gazepoint_hierarchical_location_scale_random_scale_slope(
    ...
    with pytest.raises(ValueError, match="must be numeric"):
        rs.fit_gazepoint_hierarchical_location_scale_random_scale_slope(
            ...
            "outcome",
            "participant",
            scale_cols=["condition"],
            scale_random_slope_col="condition",
            quadrature_points=3,
            maxiter=2,
            require_convergence=False,
        )
    ...
    with pytest.raises(ValueError, match="within-sample variation"):
        rs._scale_random_slope_values(frame, groups, "x", ["x"], enc)


def test_simulator_guardrails_and_known_truth_metadata():
    with pytest.raises(ValueError, match="six groups"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(n_groups=5)
    with pytest.raises(ValueError, match="three observations"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
            observations_per_group=2
        )
    with pytest.raises(ValueError, match="two finite"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(beta=(1,))
    with pytest.raises(ValueError, match="two finite"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
            gamma=(0, np.nan)
        )
    with pytest.raises(ValueError, match="positive finite"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
            tau_log_scale_slope=0
        )
    with pytest.raises(ValueError, match="positive finite"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
            tau_log_scale_intercept=np.nan
        )
    with pytest.raises(ValueError, match="strictly between"):
        rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
            rho_location_log_scale_slope=0.99
        )
    with pytest.raises(ValueError, match="strictly between"):
    ...
    data = rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
        n_groups=6, observations_per_group=3, tau_log_scale_slope=0.4, seed=99
    )
    truth = data.attrs["known_truth"]
    assert truth["tau_log_scale_slope"] == pytest.approx(0.4)
    assert np.asarray(truth["random_effect_covariance"]).shape == (3, 3)
...
def test_prediction_summary_and_certificate_type_guardrails(fitted_case, monkeypatch):
    ...
    with pytest.raises(TypeError):
        rs.summarize_gazepoint_hierarchical_location_scale_random_scale_slope(object())
    with pytest.raises(TypeError):
        rs.create_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(object())
    nonconv = replace(result, diagnostics={"converged": False})
    with pytest.raises(ValueError, match="non-converged"):
        rs.create_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(nonconv)
    assert not rs.validate_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(
        result, []
    )
    assert not rs.validate_gazepoint_hierarchical_location_scale_random_scale_slope_certificate(
    ...
    with pytest.raises(ValueError, match="Missing group"):
        rs.predict_gazepoint_hierarchical_location_scale_random_scale_slope(
    ...
    bad_data["x"] = np.nan
    with pytest.raises(ValueError, match="Random scale-slope predictor"):
...
def test_internal_fallbacks_and_objective_paths(monkeypatch, fitted_case):
    ...
    monkeypatch.setattr(rs.np.linalg, "eigvalsh", lambda x: np.array([np.nan, 1, 1]))
    _, cp, _ = rs._posterior_mode(yv, xb, zg, rv, inv, ld, max_steps=0)
    assert np.allclose(cp, np.eye(3))
    monkeypatch.setattr(rs.np.linalg, "eigvalsh", lambda x: np.array([-1.0, 1.0, 2.0]))
    monkeypatch.setattr(
        rs.np.linalg,
        "inv",
    ...
    assert (
        rs._adaptive_group_integral(yv, xb, zg, rv, inv, ld, x0, x1, x2, logw)
        == -np.inf
    )
    ...
    out = rs.fit_gazepoint_hierarchical_location_scale_random_scale_slope(
        data,
        "outcome",
        "participant",
        scale_cols=["x"],
...
def test_synthetic_known_truth_random_scale_slope_recovery():
    data = rs.simulate_gazepoint_hierarchical_location_scale_random_scale_slope(
        n_groups=18,
        observations_per_group=10,
        tau_location_intercept=0.45,
        tau_log_scale_intercept=0.20,
        tau_log_scale_slope=0.45,
        rho_location_log_scale_intercept=0.10,
        rho_location_log_scale_slope=-0.05,
        rho_log_scale_intercept_slope=0.15,
        seed=21,
    )
    result = rs.fit_gazepoint_hierarchical_location_scale_random_scale_slope(
        data,
    ...
    assert result.scale_coef[1] > 0.02
    assert result.tau_log_scale_slope > 0.08
    assert np.linalg.eigvalsh(result.random_effect_covariance).min() > 0
