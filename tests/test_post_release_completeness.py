...
def test_release_version_and_contract():
    assert gp.__version__=='0.1.7.dev0'; assert len(gp.R_EXPORTS)==406; assert len(gp.IMPLEMENTED_EXPORTS)==406; assert len(gp.PENDING_EXPORTS)==0
...
def test_archival_metadata_is_zenodo_ready_and_unambiguous():
    ...
    assert zenodo["version"] == "0.1.7.dev0"
