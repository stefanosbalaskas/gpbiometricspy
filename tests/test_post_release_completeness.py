...
def test_release_version_and_contract():
    assert gp.__version__=='0.1.5.dev0'; assert len(gp.R_EXPORTS)==406; assert len(gp.IMPLEMENTED_EXPORTS)==406; assert len(gp.PENDING_EXPORTS)==0
...
def test_visual_documentation_surface_is_committed_and_navigable():
    ...
    assert manifest['package_version']=='0.1.5.dev0'
...
def test_archival_metadata_is_zenodo_ready_and_unambiguous():
    ...
    assert zenodo["version"] == "0.1.5.dev0"
    ...
    assert "version: 0.1.4" in cff
    assert "date-released: 2026-09-06" in cff
    assert "doi: 10.5281/zenodo.22515782" in cff
